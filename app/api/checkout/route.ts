...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error('[emblem] Stripe checkout session creation failed', err);
      return NextResponse.json(
        { error: 'Could not start card checkout right now. Check STRIPE_SECRET_KEY is set correctly, or try a different payment method.' },
        { status: 502 }
      );
    }
    ...
    // Last-resort catch-all: whatever failed (misconfigured env var, DB
    // connection issue, etc.), the client always gets clean JSON back
    // instead of an empty/crashed response it can't even parse.
    console.error('[emblem] /api/checkout failed', err);
