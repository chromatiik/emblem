...
const patchSchema = z.object({
  ...
  action: z.enum(['disable', 'enable', 'ban', 'unban', 'promote', 'demote', 'ban_ip', 'unban_ip', 'exempt_ip_logging', 'unexempt_ip_logging']),
...
async function PATCHHandler(req: Request) {
  ...
  // promote/demote only ever move a user between 'user' and 'admin' — never
  // 'owner' — so admin-level auth (already checked above) is sufficient here.
  // Owner accounts are separately protected below regardless of action.
  ...
  const actions: Record<string, string> = {
    ...
    exempt_ip_logging: `UPDATE users SET ip_logging_exempt = TRUE WHERE id = $1`,
    unexempt_ip_logging: `UPDATE users SET ip_logging_exempt = FALSE WHERE id = $1`,
  };
