// The actual database implementation, deliberately WITHOUT `import
// 'server-only'`. That guard only works inside Next.js's own webpack build
// — it unconditionally throws when loaded through a plain Node/tsx process,
// which is exactly how db/migrate.ts and db/seed.ts run. Those CLI scripts
// import from here directly; the Next.js app imports through lib/db.ts
// instead, which re-exports everything below behind the server-only guard.
...
// Deliberately lazy: throwing at module-load time crashes the entire
// serverless function on cold start, before the route handler's own
// try/catch ever runs — the caller gets an empty/malformed response with
// no useful error message. Constructing the pool on first actual use means
// a missing DATABASE_URL surfaces as a normal catchable error inside
