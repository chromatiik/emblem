          ...
          <tbody>
            {logs?.map((l) => (
              <tr key={l.id} className="border-t border-white/[0.06]">
                <td className="px-4 py-3 font-medium text-ink">{l.actor_username || 'system'}</td>
                <td className="px-4 py-3 text-neutral-300">{l.action}</td>
                ...
                <td className="px-4 py-3 text-neutral-400">{new Date(l.created_at).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
